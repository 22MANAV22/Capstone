...
################################################################################
# AIRFLOW ON EC2 - AUTOMATED SETUP SCRIPT (ULTIMATE FIXED VERSION)
# Version: 2.2 - Production Ready (Config issue resolved EARLY!)
...
################################################################################
# SECTION 5: FIX CONFIG BEFORE AIRFLOW TRIES TO USE IT
################################################################################

section_header "STEP 5: PRE-CONFIGURE AIRFLOW (PREVENT CONFIG ERRORS)"

log_info "Creating directory structure..."
mkdir -p ~/airflow/{dags,logs,plugins,config}

log_info "Creating fixed airflow.cfg BEFORE database initialization..."

# Create the corrected config file with properly escaped logging format
...
log_success "Config file created with correct logging format"

# Verify config is correct
if grep -q "executor = SequentialExecutor" ~/airflow/airflow.cfg; then
    log_success "✓ Executor set to SequentialExecutor"
else
...
if grep -q "web_server_host = 0.0.0.0" ~/airflow/airflow.cfg; then
    log_success "✓ Web server host set to 0.0.0.0"
else
    log_error "Web server host not set correctly!"
    exit 1
fi

if grep -q "log_format = \[\%\%(asctime)s\]" ~/airflow/airflow.cfg; then
    log_success "✓ Logging format properly escaped"
else
    log_error "Logging format not escaped correctly!"
    exit 1
fi

################################################################################
# SECTION 6: AIRFLOW DATABASE INITIALIZATION
################################################################################

section_header "STEP 6: AIRFLOW DATABASE INITIALIZATION"

log_info "Initializing Airflow database with fixed config..."
airflow db migrate
...
log_success "Airflow version confirmed: $INSTALLED_VERSION"
